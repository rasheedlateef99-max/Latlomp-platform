...
/* Institution models — path is ../institution/models/ from routes/ */
...
router.get('/questions', async (req, res) => {
  try {
    ...
    const questions = await Question.find(filter).populate('examId', 'title type').sort({ createdAt: -1 });
...
router.post('/questions/bulk', async (req, res) => {
  try {
    ...
    if (invalid !== -1) {
      return res.status(400).json({ success: false, message: `Question ${invalid + 1} is invalid.` });
    }
...
router.get('/results', async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true, total,
      page: parseInt(page), pages: Math.ceil(total / parseInt(limit)), results
    });
...
/* ============================================
   ...
   All routes below manage the institution module
   from the main admin perspective.
============================================ */
...
router.get('/institutions/stats', async (req, res) => {
  try {
    ...
    var recentSchools = await School.find({})
...
/* ---- List all institutions with filter/search/pagination ---- */
router.get('/institutions', async (req, res) => {
  try {
    ...
    var schools = await School.find(filter)
      ...
      .select('name email status subscriptionPlan subscriptionExpiry isSuspended trialUsed ownerEmail ownerGoogleId createdAt logo');

    /* For each school get teacher + exam counts */
...
/* ---- Get single institution full detail ---- */
router.get('/institutions/:id', async (req, res) => {
  try {
    ...
    var [teachers, exams, results, subscriptions, announcements] = await Promise.all([
      ...
      SchoolExam.find({ schoolId: school._id }).sort({ createdAt: -1 }).limit(10).select('title subject status totalAttempts createdAt'),
    ...
    return res.status(200).json({
      ...
      stats: {
        teachers:  teachers.filter(function(t) { return t.isActive; }).length,
        exams:     exams.length,
        results:   results
      }
    });
...
/* ---- Activate/unsuspend institution ---- */
router.put('/institutions/:id/activate', async (req, res) => {
  try {
    ...
    var updated = await School.findByIdAndUpdate(
      req.params.id,
...
router.put('/institutions/:id/subscription', async (req, res) => {
  try {
    ...
    /* action: 'add_days' | 'expire' | 'grant_unlimited' | 'set_plan' */
    ...
    if (action === 'add_days') {
      ...
      await Subscription.create({
        schoolId:    school._id,
        plan:        school.subscriptionPlan || 'admin_grant',
        planName:    'Admin Grant (+' + daysToAdd + ' days)',
        amount:      0,
        startDate:   now,
      ...
      school.subscriptionExpiry = new Date(now.getTime() - 1000); /* 1 second ago */
      ...
      await Subscription.create({
        schoolId:    school._id,
        plan:        'unlimited',
        planName:    'Unlimited (Admin Grant)',
        amount:      0,
        startDate:   now,
        endDate:     school.subscriptionExpiry,
        status:      'active',
      ...
      return res.status(400).json({ success: false, message: 'Invalid action. Use: add_days, expire, grant_unlimited, set_plan' });
    }
...
/* ============================================
   ...
   Admin controls all pricing and limits.
...
/* ---- Update plan (price, limits, etc.) — changes apply immediately ---- */
...
router.put('/subscription-plans/:id/toggle', async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: plan.name + ' is now ' + (plan.isActive ? 'enabled' : 'disabled') + '.',
      plan
    });
...
/* ============================================
   ANNOUNCEMENTS
   Admin sends notices to one or all institutions.
============================================ */
...
router.post('/institutions/announcements', async (req, res) => {
  try {
    ...
    /* schoolId = null means send to ALL */
    if (!title || !message) {
    ...
    var announcements = await Promise.all(targetSchools.map(function(school) {
