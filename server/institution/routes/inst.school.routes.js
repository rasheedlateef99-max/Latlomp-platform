...
router.post('/onboarding', instProtect, async (req, res) => {
  try {
    ...
    /* ✅ FIX: Only sendSchoolWelcome here — sendTeacherInvite does NOT belong here */
    try {
      ...
      /* Email failure must never break onboarding */
...
router.post('/invite-teacher', instProtect, schoolAdminOnly, requireActiveSubscription, async (req, res) => {
  try {
    ...
    /* Create new invitation — token generated by pre-save hook */
    ...
    /* ✅ FIX: Use centralised email service — not inline sgMail */
    try {
      ...
      /* Email failure must never block the invite from being created */
